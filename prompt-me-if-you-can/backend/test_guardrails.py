...
class SemanticGuardTests(unittest.IsolatedAsyncioTestCase):
    async def test_harmless_small_talk_passes_without_model_call(self):
        client = _FakeClient(
            SemanticVerdict(
                category="topic_drift",
                should_block=True,
                confidence=0.99,
                reason="This fake verdict should not be used.",
            )
        )
        result = await topic_drift("hi", client)
        self.assertFalse(result["triggered"])
        self.assertIsNone(client.responses.last_call)
