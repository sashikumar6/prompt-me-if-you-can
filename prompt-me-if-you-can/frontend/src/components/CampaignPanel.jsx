...
export default function CampaignPanel({
  ...
  const unlockedHints = [
    ...level.hints
      .filter((hint) => attempts >= hint.attempt)
      .map((hint) => ({ ...hint, label: 'HINT' })),
    ...(attempts >= angleHint.attempt ? [angleHint] : []),
  ].sort((a, b) => a.attempt - b.attempt)
  const totalHints = level.hints.length + 1
  ...
  return (
    <div className="p-4 border-b border-green-900">
      ...
      <div className="flex justify-between text-[10px] text-gray-700 mb-3">
        ...
        <span>HINTS: {unlockedHints.length}/{totalHints}</span>
      </div>
      ...
      {unlockedHints.length > 0 && (
        <div className="space-y-1.5 mb-3">
          {unlockedHints.map((hint) => (
            ...
            >
              <span className="text-amber-900">{hint.label} {hint.attempt}:</span> {hint.text}
            </div>
          ))}
        </div>
      )}
