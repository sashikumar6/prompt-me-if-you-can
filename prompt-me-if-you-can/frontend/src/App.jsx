...
function reducer(state, action) {
  switch (action.type) {
    ...
    case 'HINT_REQUEST': {
      const attemptNumber = (state.levelAttempts[action.challengeId] ?? 0) + 1
      const hintResult = {
        response: 'Hint command accepted. Check the campaign mission panel — one more clue just crawled out of the wires.',
        guardrails_fired: [],
        score_update: 0,
        judge_result: null,
        blocked: false,
      ...
      return {
        ...
        lastResult: hintResult,
        error: '',
        loading: false,
        levelAttempts: {
          ...state.levelAttempts,
          [action.challengeId]: attemptNumber,
        },
    ...
    case 'ATTACK_DONE': {
      const { prompt, result, challengeId } = action
      const statuses = { ...state.guardrailStatuses }
      const reasons  = { ...state.guardrailReasons }
      let breachedDelta = 0
      let discoveryDelta = 0
      const activeLevel = LEVELS.find((level) => level.id === challengeId)
      const activeGuardrail = activeLevel?.guardrail
      ...
      for (const g of result.guardrails_fired ?? []) {
        ...
        if (GUARDRAIL_IDS.includes(g.rule) && (!activeGuardrail || g.rule === activeGuardrail)) {
...
export default function App() {
  ...
  const handleAttack = useCallback(async (prompt) => {
    ...
    if (/^(?:hint|help|\?)$/i.test(prompt.trim())) {
      dispatch({
        type: 'HINT_REQUEST',
    ...
    try {
      ...
      dispatch({ type: 'ATTACK_DONE', prompt, result, challengeId: level.id })
    } catch (err) {
